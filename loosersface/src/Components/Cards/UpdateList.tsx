...
import { useNavigate , useParams } from 'react-router-dom'; // use params is used for taking value from url 
...
type UserState= {
    currentUser: {
        _id: string;
        avatar : string; // Adjust the type of _id according to your application
...
export default function UpdateList() {
    ...
    useEffect(()=>{
          const fetchlisting= async ()=>{
             const listing = params.id;
             console.log("id which we gate from url",listing)
             const respons = await fetch(`/api/list/get/${listing}`);
 ...
 console.log(formdata)
  ...
  const removeimghandler=(urlindex : string)=>{
    ...
    const submithandler = async(e : React.MouseEvent<HTMLButtonElement, MouseEvent>)=>{
        ...
        try{
            ...
            const res = await  fetch(`/api/list/update/${params.id}`,options);
            ...
            navigate(`/listing-page/${params.id}`)
        }catch(err){
  ...
  return (
    <div>
       <h1 className='headingtext'>Update Listing</h1>
       <form>
        <div>
            ...
            <div>
                <input type="text" value={formdata.title} onChange={handlechange} id='title' placeholder='title'/>
            </div>
            <div>
                <input type="textarea" value={formdata.description} onChange={handlechange} id='description' placeholder='description'/>
            </div>
            <div>
                <input type="number" value={formdata.price} onChange={handlechange} id='price' placeholder='price' min={1}/>
            </div>
            </div>
            <div className='multiple' >
                <div>
                <select  id="category" value={formdata.category} onChange={handlechangeselect} name="category"  placeholder='category'>
                ...
                <div>
                    <select  id='subcategory' value={formdata.subcategory} onChange={handlechangeselect}  name='category' >
            ...
            <div className='buttonn'>
                ...
                <button  onClick={submithandler} className='button1'>{loading ? "Loading..." : "Update-List"}</button>
                </div>
